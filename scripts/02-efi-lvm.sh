...
wipefs --all --force /dev/sda &>>/tmp/install.log \
    && echo -e "\e[32m[OK]\e[0m  Filesystem wiped." \
    || echo -e "\e[31m[NOK]\e[0m Failed to wipe filelsystem."
...
lvcreate --yes -L 16G volgroup0 -n lv_root &>>/tmp/install.log \
...
lvcreate --yes -l 100%FREE volgroup0 -n lv_home &>>/tmp/install.log \
    && echo -e "\e[32m[OK]\e[0m  LVM logical volume created." \
    || echo -e "\e[31m[NOK]\e[0m Failed to create LVM logical volume."
